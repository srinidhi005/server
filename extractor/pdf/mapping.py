...
'Cost of Revenue':['Cost of Sales','Cost of Revenues','Cost of Sale','Cost of Revenue', 'COGs', 'Cost of goods sold'],
...
###########################################################################  2 #######################################################
        ...
        if re.search('\\b'+'millions'+'\\b',checkt1,re.I) or re.search('\\b'+'thousands'+'\\b',checkt1,re.I)  or re.search('\\b'+'per'+'\\b',checkt1,re.I) or re.search('\\b'+'par'+'\\b',checkt1,re.I) or months_var:
        ...
        with open(file_name1,encoding="utf8") as infile, open(sys.argv[3]+"/text/fileer.txt".format(out_m), "w",encoding="utf8") as outfile:
...
#########################################################################         3         #######################################
        ...
        for k, j in enumerate(main_json['period']):
            ...
            for f, b, ori, tr, co in zip(depth_lis, content_lis, original1, transformed, code):
                ...
                if unit_convers:
                    if re.search('per common share',b[0],re.I) or re.search('per share',b[0],re.I) or re.search('par value',b[0],re.I):
                        conversio=True
                        if len(b) == 1:
                            line_span1 =True
                        else:
                ...
                if inc_flag==True and len(b) != 1:
                    ...
                    if re.search('gross profit',tr,re.I) or re.search('Operating Income/loss',tr,re.I):
                        gross.insert(k,b[k+1])
                    if flagg==0:
                        if (re.search('income',b[0],re.I) or re.search('interest',b[0],re.I)) and (re.search('expense',b[0],re.I) or re.search('expenses',b[0],re.I)) and not (re.search('taxes',b[0],re.I) or re.search('tax',b[0],re.I)):
                    ...
                    elif re.search('income',b[0],re.I) and (re.search('tax',b[0],re.I) or re.search('taxes',b[0],re.I)) and (re.search('expense',b[0],re.I) or re.search('expenses',b[0],re.I)) or re.search("Provision for income taxes",tr,re.I):

                        inc_tax.insert(k,b[k+1])
                    elif re.search('^net income$',b[0],re.I) or re.search('^net income/loss$',b[0],re.I) or re.search('^net loss$',b[0],re.I) or re.search('^net loss/income$',b[0],re.I) or re.search('^net loss (income)$',b[0],re.I) or re.search('^net income (loss)$',b[0],re.I):
